...
class WeatherAPI {
  constructor() {
    ...
    // 1. `window.__OWM_API_KEY` (set by local-only `config.local.js`)
    // 2. `localStorage.getItem('OWM_API_KEY')` (optional runtime storage)
    // 3. fallback placeholder (remains unset until you provide a key)
    this.apiKey = (typeof window !== "undefined" && window.__OWM_API_KEY) ||

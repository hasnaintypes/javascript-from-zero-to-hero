...
// Declare WeatherAPI and WeatherIcons variables
const WeatherAPI = require("./weather-api")
const WeatherIcons = require("./weather-icons")
...
class WeatherApp {
  ...
  attachEventListeners() {
    ...
    this.favoritesList.addEventListener("click", (e) => this.handleFavoriteClick(e))
  ...
  checkAPIConfiguration() {
    if (!this.weatherAPI.isConfigured()) {
      this.showError("API key not configured. Please add your OpenWeatherMap API key to weather-api.js")
      return false
    }
    return true
  }
  ...
  async loadWeatherByCoords(lat, lon) {
    try {
      this.showLoading()
      const weatherData = await this.weatherAPI.getCompleteWeatherData(lat, lon)
  ...
  displayCurrentWeather(current, uv, airPollution) {
    const formatted = this.weatherAPI.formatWeatherData(current, this.currentUnits)

    // Update main weather display
    this.currentWeatherIcon.textContent = WeatherIcons.getIconByCode(current.weather[0].icon)
    ...
    this.windSpeed.textContent = formatted.windSpeed + " " + WeatherIcons.getWindDirection(current.wind.deg)
        ...
        return `
                <div class="daily-item">
                    ...
                    <div class="daily-temps">
                        <span class="daily-high">${Math.round(day.maxTemp)}${tempUnit}</span>
                        <span class="daily-low">${Math.round(day.minTemp)}${tempUnit}</span>
                    </div>
  ...
  groupForecastByDay(forecastList) {
    ...
    forecastList.forEach((item) => {
      ...
      days[dateKey].maxTemp = Math.max(days[dateKey].maxTemp, item.main.temp_max)
      days[dateKey].minTemp = Math.min(days[dateKey].minTemp, item.main.temp_min)
  ...
  toggleUnits() {
    ...
    if (this.currentLocation) {
      this.loadWeatherByCoords(this.currentLocation.lat, this.currentLocation.lon)
    }
  }
  ...
  async refreshWeather() {
    ...
    try {
      await this.loadWeatherByCoords(this.currentLocation.lat, this.currentLocation.lon)
  ...
  async addToFavorites() {
    ...
    try {
      ...
      const locationInfo = await this.weatherAPI.getCityName(this.currentLocation.lat, this.currentLocation.lon)
      ...
      const exists = this.favorites.some(
        (fav) => Math.abs(fav.lat - favorite.lat) < 0.01 && Math.abs(fav.lon - favorite.lon) < 0.01,
      )
      ...
      .map((favorite) => {
        ...
        const temp =
          this.currentUnits === "metric"
            ? favorite.temp
            : Math.round(this.weatherAPI.convertTemperature(favorite.temp, "celsius", "fahrenheit"))
  ...
  updateWeatherBackground(conditionId, isDay) {
    ...
    document.querySelector(".current-weather::before").style.background = gradient
  }
  ...
  handleKeyboardShortcuts(e) {
    ...
    // U to toggle units
    if (e.key === "u" && !e.ctrlKey && !e.metaKey && document.activeElement !== this.searchInput) {
      this.toggleUnits()
    }
